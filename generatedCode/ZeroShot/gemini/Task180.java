    ...
    public static Map<Integer, List<Integer>> reparentTree(Map<Integer, List<Integer>> tree, int newRoot) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int node : tree.keySet()) {
                if (tree.getOrDefault(node, new ArrayList<>()).contains(curr) && !parentMap.containsKey(node)) {
                    newTree.get(curr).add(node);
                    parentMap.put(node, curr);
                    q.offer(node);
                } else if (tree.getOrDefault(curr, new ArrayList<>()).contains(node) && !parentMap.containsKey(node)) {
                    newTree.get(curr).add(node);
                    parentMap.put(node, curr);
                    q.offer(node);
                }
