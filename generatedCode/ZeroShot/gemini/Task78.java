...
public class Task78 {
    ...
    public static String generateAndExportRsaPrivateKey(String passphrase) throws NoSuchAlgorithmException, OperatorCreationException, IOException, PKCSException, InvalidKeySpecException {
        ...
        if (passphrase != null && !passphrase.isEmpty()) { // Encrypt with passphrase if provided
            //PKCS8EncryptedPrivateKeyInfoBuilder pkcs8EncryptedPrivateKeyInfoBuilder = new PKCS8EncryptedPrivateKeyInfoBuilder(privateKey);
            //JcePKCSPBEOutputEncryptorBuilder encryptorBuilder = new JcePKCSPBEOutputEncryptorBuilder(PKCSObjectIdentifiers.pbeWithSHA256And256BitAES_CBC_BC);
            //encryptorBuilder.setPassword(passphrase.toCharArray());
            //PKCS8EncryptedPrivateKeyInfo encryptedPrivateKeyInfo = pkcs8EncryptedPrivateKeyInfoBuilder.build(encryptorBuilder.build());

            JcaPEMWriter pemWriter = new JcaPEMWriter(new StringWriter());
            JcaPKCS8Generator gen = new JcaPKCS8Generator(privateKey, JcePKCSPBEOutputEncryptorBuilder.getDefaultPBEStringEncryptorBuilder(passphrase).build());

            pemWriter.writeObject(gen);
            pemWriter.flush();
            String privateKeyPEM = pemWriter.getWriter().toString();
            pemWriter.close();
