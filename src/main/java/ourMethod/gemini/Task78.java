package ourMethod.gemini;// NOTE: This code requires the Bouncy Castle library.
...
public class Task78 {
    ...
    public static String generateOpenSSHPrivateKey(int keySize) throws NoSuchAlgorithmException, IOException {
        ...
        AsymmetricKeyParameter bcPrivateKey = (AsymmetricKeyParameter) new JcaPEMKeyConverter().getPrivateKey((PrivateKeyInfo) keyPair.getPrivate());

        byte[] opensshPrivateKeyBytes = encodePrivateKey(bcPrivateKey);
    ...
    private static byte[] encodePrivateKey(AsymmetricKeyParameter bcPrivateKey) {
        return null;
    }
